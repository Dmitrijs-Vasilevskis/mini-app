...
export function UnoGameStateEvents(
    ...
    const directionListener = $(gameState).listen(
        "direction",
    ...
    const activeColorListener = $(gameState).listen(
        "activeColor",
        (color: Color) => {
    ...
    const unoPendingPlayerListener = $(gameState).listen("unoPendingPlayerId", (currPendingPlayerId: string) => {
    ...
    const topDiscardCardListener = $(gameState).listen("topDiscardCard", ((card: CardSchema | null) => {
        if (!card) return;
    ...
    unlisteners.push(
        directionListener,
        activeColorListener,
        unoPendingPlayerListener,
        topDiscardCardListener
    );
